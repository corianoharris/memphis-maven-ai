...
async function getAIResponse(prompt, contextPages = [], targetLanguage = 'en') {
  try {
    ...
    const languagePrompts = {
      'en': {
        intro: "You are Beale, a friendly Memphis city services assistant. You're helpful, knowledgeable, and speak in a casual, human way.",
        instructions: "Answer this question about Memphis city services:",
        fallback: "Hi! I'm Beale and I'm here to help with Memphis city services. For specific questions, call 211 for community services or 311 at (901) 636-6500."
      },
      'es': {
        intro: "Eres Beale, un asistente amigable de servicios de la ciudad de Memphis. Eres servicial, conocedor y hablas de manera casual y humana. IMPORTANTE: Responde SIEMPRE en español, no en inglés.",
        instructions: "Responde esta pregunta sobre servicios de la ciudad de Memphis SOLO en español:",
        fallback: "¡Hola! Soy Beale y estoy aquí para ayudar con los servicios de la ciudad de Memphis. Para preguntas específicas, llama al 211 para servicios comunitarios o al 311 al (901) 636-6500."
      },
      'ar': {
        intro: "أنت Beale، مساعد ودود لخدمات مدينة ممفيس. أنت مفيد ومطلع وتتحدث بطريقة عادية وبشرية. مهم: أجب دائماً باللغة العربية، وليس الإنجليزية.",
        instructions: "أجب على هذا السؤال حول خدمات مدينة ممفيس باللغة العربية فقط:",
        fallback: "مرحباً! أنا Beale وأنا هنا للمساعدة في خدمات مدينة ممفيس. للأسئلة المحددة، اتصل بـ 211 للخدمات المجتمعية أو 311 على (901) 636-6500."
      }
    };
    ...
    const simplePrompt = `${langConfig.intro} ${langConfig.instructions} "${prompt}"

${context ? `Relevant services: ${context}` : ''}

Give a helpful answer under 80 words. Include contact info if relevant.
  ...
  try {
    ...
    if (lowerQuestion.includes('pothole') || lowerQuestion.includes('bache') || lowerQuestion.includes('street') || lowerQuestion.includes('road') || lowerQuestion.includes('calle') || lowerQuestion.includes('carretera')) {
      if (detectedLang === 'spa') {
        ...
        fallbackAnswer = "Hey y'all! I'm Beale, straight from Beale Street! For reporting potholes, here's the BEST approach:\n\n**Recommended Solution:**\n1. Call 311 at (901) 636-6500 (fastest response)\n2. Visit memphistn.gov online form (for detailed reports)\n3. Provide exact location and description\n\n**Why this works best:** Phone calls get immediate attention and faster repair scheduling.\n**Alternative:** Use the online form for non-urgent reports\n**Timeline:** Usually repaired within 3-5 business days!\n\nWe'll get those streets smooth as Beale Street!";
      }
    } else {
      // General fallback
      if (detectedLang === 'spa') {
        ...
        fallbackAnswer = "Hey y'all! I'm Beale, straight from Beale Street! For the BEST solution to your issue, call 211 for community services or 311 at (901) 636-6500. I can recommend the most effective approach for any Memphis city service! Let's keep Memphis smooth!";
      }
    }
...
async function safeProcessQuestion(question, similarPages = [], preferredLanguage = 'en') {
  try {
    ...
    const         fallbackMessages = {
          en: "Sorry y'all, I'm having some technical hiccups right now. Please try calling Memphis 311 at (901) 636-6500 for immediate assistance.",
